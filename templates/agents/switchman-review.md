...
8. T2（收尾交接）：不再开启新阶段；完成手头当前单元后，在最终消息内嵌紧凑交接块（已完成/剩余工作/Next steps）。T3（立即交接）：不再读新文件，保存当前状态，交接块标 progress: partial，立即返回。
9. 最终消息以 HANDOFF: inline · progress: n/m · next: <一句话> 结尾。
